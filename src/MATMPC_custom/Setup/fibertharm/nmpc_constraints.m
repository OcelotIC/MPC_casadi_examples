...
nu = settings.nu_lambda;        % Number of inputs
...
% Bounds on joint angles and speeds
q_min = [-pi; -pi/2; -pi/2];%[-10*pi ; -10*pi/3 ; -10*pi/3];
q_max = [2*pi; pi/2; pi/2];%[10*pi ; 10*pi/3 ;10 * pi/3];

dq_min = [-pi; -pi; -pi];
dq_max = [pi; pi; pi];
%% Collect state bounds
lb_x = [x_min; y_min; z_min; rp_min; rp_min; q_min; dq_min];
ub_x = [x_max; y_max; z_max; rp_max; rp_max; q_max; dq_max];
% lb_x = -inf(11,1);
% ub_x =  inf(11,1);
