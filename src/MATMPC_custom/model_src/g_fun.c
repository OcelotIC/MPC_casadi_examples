...
/* g_fun:(states[20],controls[4],params[0],xdot[20],alg[0])->(zfun) */
...
void mex_g_fun(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+20, casadi_s1, w+45);
  if (--argc>=0) arg[2] = casadi_from_mex(argv[2], w+24, casadi_s2, w+45);
  if (--argc>=0) arg[3] = casadi_from_mex(argv[3], w+24, casadi_s0, w+45);
  if (--argc>=0) arg[4] = casadi_from_mex(argv[4], w+44, casadi_s2, w+45);
  casadi_real* res[1] = {0};
  --resc;
  res[0] = w+44;
  i = g_fun(arg, res, iw, w+45, 0);
